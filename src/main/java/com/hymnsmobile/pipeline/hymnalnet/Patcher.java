...
public class Patcher {
  ...
  private Set<Hymn.Builder> builders;

  @Inject
  public Patcher(@HymnalNet Set<Hymn> hymns) {
    this.hymns = hymns;
  }

  public void patch() {
    this.builders = hymns.stream().map(Hymn::toBuilder).collect(Collectors.toSet());
    ...
    // relevants map.
    this.builders.stream().filter(builder -> builder.getReference().getNumber().matches("\\d+b"))
    ...
    this.hymns.clear();
    this.hymns.addAll(builders.stream().map(Hymn.Builder::build).collect(Collectors.toSet()));
  }

  /**
   * h/1351 should also map to ht/1351,so we need to update all songs in that graph.
   */
  private void fix_h1351() {
   ...
   */
  private void fix_nt575_ns34_h711() {
    fix_nt575();
    fix_ns34();
    fix_h711();
  }
   ...
   */
  private void fix_ch9575_chnt575c() {
   ...
   */
  private void fix_h635_h481_h631() {
   ...
   */
  private void fix_ns59_ns110_ns111() {
   ...
   */
  private void fix_danglingReferences() {
  ...
  private void removeRelevants(SongReference.Builder songReference,
  ...
  private void addLanguages(SongReference.Builder songReference, SongLink.Builder... languages) {
    addSongLink(Hymn.getDescriptor().findFieldByName("languages"), songReference, languages);
  }

  private void addRelevants(SongReference.Builder songReference, SongLink.Builder... relevants) {
    addSongLink(Hymn.getDescriptor().findFieldByName("relevants"), songReference, relevants);
  }

  private void addSongLink(FieldDescriptor field, SongReference.Builder songReference,
  ...
  private void resetLanguages(SongReference.Builder english, SongLink.Builder... languages) {
    resetSongLinks(Hymn.getDescriptor().findFieldByName("languages"), english, "English",
        languages);
  }

  private void resetRelevants(SongReference.Builder originalTune, SongLink.Builder... relevants) {
    resetSongLinks(Hymn.getDescriptor().findFieldByName("relevants"), originalTune, "Original Tune",
        relevants);
  }

  private void resetSongLinks(FieldDescriptor field, SongReference.Builder original,
  ...
  private Hymn.Builder getHymn(SongReference.Builder songReference) {
    return getHymn(songReference.build());
  }

  private Hymn.Builder getHymn(SongReference songReference) {
    ImmutableList<Hymn.Builder> builder = builders.stream().filter(
